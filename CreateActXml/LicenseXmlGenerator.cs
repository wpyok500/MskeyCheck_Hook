using System;
using System.Collections.Generic;
using System.IO;
using System.Text;
using System.Xml;

namespace CreateActXml
{
    public class LicenseXmlGenerator
    {
        #region 原类保留的配置字典（与GetHResultApi完全一致）
        private static Dictionary<string, string> ClaimsDic_2005_SPC = new Dictionary<string, string>
        {
            { "SPVersion", "1.0" },
            { "SPAssuranceLevel", "urn:msft:sl/Assurance/SLS-Default/1.0" },
            { "otherInfoPublic.licenseCategory", "msft:sl/SPC/ACTIVATED/PUBLIC" },
            { "otherInfoPrivate.licenseCategory", "msft:sl/SPC/ACTIVATED/PRIVATE" },
            { "otherInfoPublic.sysprepAction", "rearm" },
            { "otherInfoPrivate.sysprepAction", "rearm" },
            { "otherInfoPublic.spcActivationGroup", "msft:Windows/6.0/SPC/Retail" },
            { "otherInfoPrivate.spcActivationGroup", "msft:Windows/6.0/SPC/Retail" }
        };

        private static Dictionary<string, string> UseKeyDic_2005_SPC = new Dictionary<string, string>();

        private static Dictionary<string, string> ClaimsDic_2005_PKC = new Dictionary<string, string>
        {
            { "ProductKey", "RFF6G-C3YCJ-F9CHT-KD6MJ-24P46" },
            { "ProductKeyType", "msft:rm/algorithm/pkey/2005" },
            { "ProductKeyActConfigId", "msft2005:c619d61c-c2f2-40c3-ab3f-c5924314b0f3&IOLFrdsAAAAAAAAA" }
        };

        private static Dictionary<string, string> UseKeyDic_2005_PKC = new Dictionary<string, string>();

        // PL(PublishLicense)直接使用原类的Win_EUL配置中的PublishLicense值
        private static Dictionary<string, string> UseKeyDic_2005_Win_EUL = new Dictionary<string, string>
        {
            { "PublishLicense", "<?xml version=\"1.0\" encoding=\"utf-8\"?><rg:licenseGroup xmlns:rg=\"urn:mpeg:mpeg21:2003:01-REL-R-NS\"><r:license xmlns:r=\"urn:mpeg:mpeg21:2003:01-REL-R-NS\" licenseId=\"{82eb9025-c25f-4dd6-a035-d6994302beb2}\" xmlns:sx=\"urn:mpeg:mpeg21:2003:01-REL-SX-NS\" xmlns:mx=\"urn:mpeg:mpeg21:2003:01-REL-MX-NS\" xmlns:sl=\"http://www.microsoft.com/DRM/XrML2/SL/v2\" xmlns:tm=\"http://www.microsoft.com/DRM/XrML2/TM/v2\"><r:title>Windows(R) 7 Publishing License (Public)</r:title><r:grant><r:forAll varName=\"account\"><r:anXmlExpression>/tm:account/tm:identity[@type='urn:msft:tm:identity:hwid' or @type='urn:msft:tm:identity:volume' or @type='urn:msft:tm:identity:oem']</r:anXmlExpression></r:forAll><r:forAll varName=\"accountKey\"><r:propertyPossessor><tm:account varRef=\"account\"/><r:trustedRootIssuers><r:keyHolder><r:info><KeyValue xmlns=\"http://www.w3.org/2000/09/xmldsig#\"><RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue></KeyValue></r:info></r:keyHolder></r:trustedRootIssuers></r:propertyPossessor></r:forAll><r:forAll varName=\"productId\"><r:anXmlExpression>/sl:productId/sl:pid</r:anXmlExpression></r:forAll><r:forAll varName=\"binding\"></r:forAll><r:keyHolder><r:info><KeyValue xmlns=\"http://www.w3.org/2000/09/xmldsig#\"><RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue></KeyValue></r:info></r:keyHolder><r:issue/><r:grant><r:forAll varName=\"application\"><r:anXmlExpression>editionId[@value=\"\" or @value=\"Ultimate\"]</r:anXmlExpression></r:forAll><r:forAll varName=\"appid\"><r:propertyPossessor><tm:application varRef=\"application\"/><r:trustedRootIssuers><r:keyHolder><r:info><KeyValue xmlns=\"http://www.w3.org/2000/09/xmldsig#\"><RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue></KeyValue></r:info></r:keyHolder></r:trustedRootIssuers></r:propertyPossessor></r:forAll><r:keyHolder varRef=\"accountKey\"/><sl:runSoftware/><sl:appId varRef=\"appid\"/><r:allConditions><r:allConditions><sl:productPolicies xmlns:sl=\"http://www.microsoft.com/DRM/XrML2/SL/v2\"><sl:priority>400</sl:priority><sl:policyStr name=\"Security-SPP-Reserved-Family\" attributes=\"override-only\">Ultimate</sl:policyStr></sl:productPolicies><sl:proxyExecutionKey xmlns:sl=\"http://www.microsoft.com/DRM/XrML2/SL/v2\"></sl:proxyExecutionKey></r:allConditions><mx:renderer><sl:binding varRef=\"binding\"/><sl:productId varRef=\"productId\"/></mx:renderer></r:allConditions></r:grant><r:allConditions><sl:businessRules xmlns:sl=\"http://www.microsoft.com/DRM/XrML2/SL/v2\"></sl:businessRules></r:allConditions></r:grant><r:issuer><Signature xmlns=\"http://www.w3.org/2000/09/xmldsig#\"><SignedInfo><CanonicalizationMethod Algorithm=\"http://www.microsoft.com/xrml/lwc14n\"/><SignatureMethod Algorithm=\"http://www.w3.org/2000/09/xmldsig#rsa-sha1\"/><Reference><Transforms><Transform Algorithm=\"urn:mpeg:mpeg21:2003:01-REL-R-NS:licenseTransform\"/><Transform Algorithm=\"http://www.microsoft.com/xrml/lwc14n\"/></Transforms><DigestMethod Algorithm=\"http://www.w3.org/2000/09/xmldsig#sha1\"/><DigestValue>latzLE7Y8EzdqTUgmh2jIWH9U4A=</DigestValue></Reference></SignedInfo><SignatureValue></SignatureValue><KeyInfo><KeyValue><RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue></KeyValue></KeyInfo></Signature><r:details><r:timeOfIssue>2010-11-20T13:36:08Z</r:timeOfIssue></r:details></r:issuer><r:otherInfo xmlns:r=\"urn:mpeg:mpeg21:2003:01-REL-R-NS\"><tm:infoTables xmlns:tm=\"http://www.microsoft.com/DRM/XrML2/TM/v2\"><tm:infoList tag=\"#global\"><tm:infoStr name=\"licenseType\">msft:sl/PL/GENERIC/PUBLIC</tm:infoStr><tm:infoStr name=\"licenseVersion\">2.0</tm:infoStr><tm:infoStr name=\"licensorUrl\">http://licensing.microsoft.com</tm:infoStr><tm:infoStr name=\"licenseCategory\">msft:sl/PL/GENERIC/PUBLIC</tm:infoStr><tm:infoStr name=\"productSkuId\">{c619d61c-c2f2-40c3-ab3f-c5924314b0f3}</tm:infoStr><tm:infoStr name=\"privateCertificateId\">{75d3bbaa-65e0-4bb0-aa85-61ed95ddee95}</tm:infoStr><tm:infoStr name=\"applicationId\">{55c92734-d682-4d71-983e-d6ec3f16059f}</tm:infoStr><tm:infoStr name=\"productName\">Windows(R) 7</tm:infoStr><tm:infoStr name=\"productAuthor\">Microsoft Corporation</tm:infoStr><tm:infoStr name=\"productDescription\">Windows Operating System - Windows(R) 7</tm:infoStr><tm:infoStr name=\"clientIssuanceCertificateId\">{4961cc30-d690-43be-910c-8e2db01fc5ad}</tm:infoStr><tm:infoStr name=\"referralData\">ReferralId=000000;PartnerId=00000000-0000-0000-0000-000000000000</tm:infoStr></tm:infoList></tm:infoTables></r:otherInfo></r:license><r:license xmlns:r=\"urn:mpeg:mpeg21:2003:01-REL-R-NS\" licenseId=\"{75d3bbaa-65e0-4bb0-aa85-61ed95ddee95}\" xmlns:sx=\"urn:mpeg:mpeg21:2003:01-REL-SX-NS\" xmlns:mx=\"urn:mpeg:mpeg21:2003:01-REL-MX-NS\" xmlns:sl=\"http://www.microsoft.com/DRM/XrML2/SL/v2\" xmlns:tm=\"http://www.microsoft.com/DRM/XrML2/TM/v2\"><r:title>Windows(R) 7 Publishing License (Private)</r:title><r:grant><r:encryptedGrant><EncryptionMethod xmlns=\"http://www.w3.org/2001/04/xmlenc#\" Algorithm=\"http://www.w3.org/2001/04/xmlenc#aes128-cbc\"/><KeyInfo xmlns=\"http://www.w3.org/2000/09/xmldsig#\"><EncryptedKey xmlns=\"http://www.w3.org/2001/04/xmlenc#\"><EncryptionMethod Algorithm=\"http://www.w3.org/2001/04/xmlenc#rsa-oaep-mgf1p\" /><KeyInfo xmlns=\"http://www.w3.org/2000/09/xmldsig#\"><KeyValue xmlns=\"http://www.w3.org/2000/09/xmldsig#\"><RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue></KeyValue></KeyInfo><CipherData><CipherValue></CipherValue></CipherData></EncryptedKey></KeyInfo><CipherData xmlns=\"http://www.w3.org/2001/04/xmlenc#\"><CipherValue></CipherValue></CipherData></r:encryptedGrant></r:grant><r:issuer><Signature xmlns=\"http://www.w3.org/2000/09/xmldsig#\"><SignedInfo><CanonicalizationMethod Algorithm=\"http://www.microsoft.com/xrml/lwc14n\"/><SignatureMethod Algorithm=\"http://www.w3.org/2000/09/xmldsig#rsa-sha1\"/><Reference><Transforms><Transform Algorithm=\"urn:mpeg:mpeg21:2003:01-REL-R-NS:licenseTransform\"/><Transform Algorithm=\"http://www.microsoft.com/xrml/lwc14n\"/></Transforms><DigestMethod Algorithm=\"http://www.w3.org/2000/09/xmldsig#sha1\"/><DigestValue>ZtsHovP7OBVt1LI2tHvbgTTqeE0=</DigestValue></Reference></SignedInfo><SignatureValue></SignatureValue><KeyInfo><KeyValue><RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue></KeyValue></KeyInfo></Signature><r:details><r:timeOfIssue>2010-11-20T13:36:08Z</r:timeOfIssue></r:details></r:issuer><r:otherInfo xmlns:r=\"urn:mpeg:mpeg21:2003:01-REL-R-NS\"><tm:infoTables xmlns:tm=\"http://www.microsoft.com/DRM/XrML2/TM/v2\"><tm:infoList tag=\"#global\"><tm:infoStr name=\"licenseType\">msft:sl/PL/GENERIC/PRIVATE</tm:infoStr><tm:infoStr name=\"licenseVersion\">2.0</tm:infoStr><tm:infoStr name=\"licensorUrl\">http://licensing.microsoft.com</tm:infoStr><tm:infoStr name=\"licenseCategory\">msft:sl/PL/GENERIC/PRIVATE</tm:infoStr><tm:infoStr name=\"publicCertificateId\">{82eb9025-c25f-4dd6-a035-d6994302beb2}</tm:infoStr><tm:infoStr name=\"clientIssuanceCertificateId\">{4961cc30-d690-43be-910c-8e2db01fc5ad}</tm:infoStr></tm:infoList></tm:infoTables></r:otherInfo></r:license></rg:licenseGroup>" }
        };
        #endregion

        #region 核心SOAP信封构建方法（复用原类逻辑）
        private static string BuildEnvelope(string tokenType, Dictionary<string, string> useKeyDic, Dictionary<string, string> claimsDic)
        {
            using (var memoryStream = new MemoryStream())
            {
                var encoding = new UTF8Encoding(false);
                using (var xmlTextWriter = new XmlTextWriter(memoryStream, encoding))
                {
                    xmlTextWriter.Formatting = Formatting.Indented; // 格式化XML，便于阅读
                    xmlTextWriter.WriteStartDocument();
                    // 写入SOAP信封根节点及命名空间
                    xmlTextWriter.WriteStartElement("soap", "Envelope", "http://schemas.xmlsoap.org/soap/envelope/");
                    xmlTextWriter.WriteAttributeString("xmlns", "soapenc", null, "http://schemas.xmlsoap.org/soap/encoding/");
                    xmlTextWriter.WriteAttributeString("xmlns", "xsd", null, "http://www.w3.org/2001/XMLSchema");
                    xmlTextWriter.WriteAttributeString("xmlns", "xsi", null, "http://www.w3.org/2001/XMLSchema-instance");

                    // 写入SOAP Body
                    xmlTextWriter.WriteStartElement("soap", "Body", null);
                    // 写入请求安全令牌节点
                    xmlTextWriter.WriteStartElement("RequestSecurityToken", "http://schemas.xmlsoap.org/ws/2004/04/security/trust");

                    // 写入TokenType
                    xmlTextWriter.WriteStartElement("TokenType");
                    xmlTextWriter.WriteString(tokenType);
                    xmlTextWriter.WriteEndElement();

                    // 写入RequestType
                    xmlTextWriter.WriteStartElement("RequestType");
                    xmlTextWriter.WriteString("http://schemas.xmlsoap.org/ws/2004/04/security/trust/Issue");
                    xmlTextWriter.WriteEndElement();

                    // 写入UseKey节点
                    xmlTextWriter.WriteStartElement("UseKey");
                    xmlTextWriter.WriteStartElement("Values");
                    if (useKeyDic.Count > 0)
                    {
                        xmlTextWriter.WriteAttributeString("xmlns", "q1", null, "http://schemas.xmlsoap.org/ws/2004/04/security/trust");
                        xmlTextWriter.WriteAttributeString("soapenc", "arrayType", null, $"q1:TokenEntry[{useKeyDic.Count}]");
                        foreach (var kvp in useKeyDic)
                        {
                            xmlTextWriter.WriteStartElement("TokenEntry");
                            xmlTextWriter.WriteStartElement("Name");
                            xmlTextWriter.WriteString(kvp.Key);
                            xmlTextWriter.WriteEndElement();
                            xmlTextWriter.WriteStartElement("Value");
                            xmlTextWriter.WriteString(kvp.Value);
                            xmlTextWriter.WriteEndElement();
                            xmlTextWriter.WriteEndElement();
                        }
                    }
                    else
                    {
                        xmlTextWriter.WriteAttributeString("xsi", "nil", null, "true");
                    }
                    xmlTextWriter.WriteEndElement(); // Values
                    xmlTextWriter.WriteEndElement(); // UseKey

                    // 写入Claims节点
                    xmlTextWriter.WriteStartElement("Claims");
                    xmlTextWriter.WriteStartElement("Values");
                    xmlTextWriter.WriteAttributeString("xmlns", "q1", null, "http://schemas.xmlsoap.org/ws/2004/04/security/trust");
                    xmlTextWriter.WriteAttributeString("soapenc", "arrayType", null, $"q1:TokenEntry[{claimsDic.Count}]");
                    foreach (var kvp in claimsDic)
                    {
                        xmlTextWriter.WriteStartElement("TokenEntry");
                        xmlTextWriter.WriteStartElement("Name");
                        xmlTextWriter.WriteString(kvp.Key);
                        xmlTextWriter.WriteEndElement();
                        xmlTextWriter.WriteStartElement("Value");
                        xmlTextWriter.WriteString(kvp.Value);
                        xmlTextWriter.WriteEndElement();
                        xmlTextWriter.WriteEndElement();
                    }
                    xmlTextWriter.WriteEndElement(); // Values
                    xmlTextWriter.WriteEndElement(); // Claims

                    // 关闭所有节点
                    xmlTextWriter.WriteEndElement(); // RequestSecurityToken
                    xmlTextWriter.WriteEndElement(); // soap:Body
                    xmlTextWriter.WriteEndElement(); // soap:Envelope
                    xmlTextWriter.Flush();

                    // 转换为字符串返回
                    return encoding.GetString(memoryStream.ToArray());
                }
            }
        }
        #endregion

        #region 对外公开的XML生成方法
        /// <summary>
        /// 生成SPC XML（SecurityProcessorCertificate）
        /// </summary>
        /// <returns>带SOAP信封的SPC XML字符串</returns>
        public static string GenerateSpcXml()
        {
            return BuildEnvelope("SPC", UseKeyDic_2005_SPC, ClaimsDic_2005_SPC);
        }

        /// <summary>
        /// 生成PKC XML（ProductKeyCertificate）
        /// </summary>
        /// <returns>带SOAP信封的PKC XML字符串</returns>
        public static string GeneratePkcXml()
        {
            return BuildEnvelope("PKC", UseKeyDic_2005_PKC, ClaimsDic_2005_PKC);
        }

        /// <summary>
        /// 生成PL XML（PublishLicense，发布许可证）
        /// </summary>
        /// <returns>纯PL XML字符串（无SOAP信封，与原类配置一致）</returns>
        public static string GeneratePlXml()
        {
            // PL为独立的许可证XML，无SOAP信封，直接返回原配置中的值
            return UseKeyDic_2005_Win_EUL["PublishLicense"];
        }
        #endregion

        #region 测试方法（可选，验证生成结果）
        public static void TestGenerateAllXml()
        {
            var spcXml = GenerateSpcXml();
            var pkcXml = GeneratePkcXml();
            var plXml = GeneratePlXml();

            // 输出到控制台
            Console.WriteLine("========== SPC XML ==========\n");
            Console.WriteLine(spcXml);
            Console.WriteLine("\n========== PKC XML ==========\n");
            Console.WriteLine(pkcXml);
            Console.WriteLine("\n========== PL XML ==========\n");
            Console.WriteLine(plXml.Substring(0, 500) + "..."); // PL过长，只输出前500字符

            // 也可保存到文件
            File.WriteAllText("SPC.xml", spcXml, Encoding.UTF8);
            File.WriteAllText("PKC.xml", pkcXml, Encoding.UTF8);
            File.WriteAllText("PL.xml", plXml, Encoding.UTF8);
            Console.WriteLine("\nXML文件已保存到程序运行目录！");
        }
        #endregion
    }

}