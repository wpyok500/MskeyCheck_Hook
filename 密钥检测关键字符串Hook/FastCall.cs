...
namespace 密钥检测关键字符串Hook
{
    public static class FastCall
    {
        ...
        /// 申请虚拟内存
        /// </summary>
        /// <param name="hProcess">要分配的区域的起始地址</param>
        /// <param name="size">区域的大小</param>
        /// <param name="flAllocationType">内存分配的类型</param>
        /// <param name="flProtect">要分配的页面区域的内存保护类型</param>
        /// <returns></returns>
        ...
        public static IntPtr WrapStdCallInFastCall(IntPtr stdCallPtr)//用于HOOK
        {
            ...
            Marshal.WriteIntPtr(result, 5, stdCallPtr); //5个字节 创建jmp的时候 先加了个E9 然后后面是4个字节的指针
            return result;
        }
