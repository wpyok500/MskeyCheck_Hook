...
namespace 密钥检测关键字符串Hook
{
    public unsafe class Hook_API
    {
        ...
        public Hook_API(IntPtr target, IntPtr hook)
        {
            ...
            m_OriginalBytes = new byte[5];//32位要5个字节才行 //64位的指针是8字节的
        ...
        static void ProtectionSafeMemoryCopy(byte[] dest, IntPtr source)
        {
            ...
            {
                dest[i] = Marshal.ReadByte(source,i);
            }
        ...
        static void ProtectionSafeMemoryCopy(IntPtr dest, byte[] source)
        {
            ...
            {
                 Marshal.WriteByte(dest, i, source[i]);
            }
